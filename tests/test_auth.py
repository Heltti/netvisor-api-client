...
class TestNetvisorAuth(object):
    ...
    def http_request(self, request, auth):
        (flexmock(auth).should_receive("make_transaction_id").and_return("123456"))
        (
            flexmock(auth)
    ...
    def test_adds_sender_header_to_request(self, http_request):
        assert http_request.headers["X-Netvisor-Authentication-Sender"] == "Testiclient"
    ...
    def test_adds_organization_id_header_to_request(self, http_request):
        assert http_request.headers["X-Netvisor-Organisation-ID"] == "1967543-8"

    def test_adds_transaction_id_header_to_request(self, http_request):
        assert (
            http_request.headers["X-Netvisor-Authentication-TransactionId"] == "123456"
        )
    ...
    def test_adds_partner_id_header_to_request(self, http_request):
        assert http_request.headers["X-Netvisor-Authentication-PartnerId"] == "xxx_yyy"
